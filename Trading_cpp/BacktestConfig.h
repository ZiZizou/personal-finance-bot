...
struct PositionSizing {
    enum class Method {
        ...
        ConfidenceWeighted   // Scale position by signal confidence
    };
    ...
    double calculateFraction(double winRate = 0.5, double avgWinLossRatio = 1.0,
                           double atr = 0.0, double price = 0.0,
        ...
        switch (method) {
            ...
            case Method::ConfidenceWeighted: {
                // confidence=1.0 → 100% of baseFraction
                // confidence=0.5 → 75%
                // confidence=0.0 → 50%
                double clampedConf = std::max(0.0, std::min(1.0, confidence));
                fraction = fixedFraction * (0.5 + 0.5 * clampedConf);
                break;
            }
        }
